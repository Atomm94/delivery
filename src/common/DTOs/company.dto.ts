...
export class UpdateCompanyDataDto  {
  ...
  @IsOptional()
  @IsString({ message: 'phone number must be a string' })
  @Matches(/^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$/, { message: 'phone number must be a valid phone number' })
  phone_number?: string;
  ...
  // Completion fields (all optional)
  @ApiProperty({ example: '123456789', required: false })
  @IsOptional()
  @IsString({ message: 'ITN must be a string' })
  ITN?: string;
...
class CompanyDriverSignUpDto extends OmitType(DriversSignUpDto, ['password'] as const) {}

class CompanyDriverDto extends IntersectionType(
  CompanyDriverSignUpDto,
